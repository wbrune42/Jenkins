...
RUN apt-get update \
      && apt-get install -y sudo curl libltdl7 virtualbox-5.1 linux-headers-amd64
...
COPY jobs/1-jenkins-seed-job.xml /usr/share/jenkins/ref/jobs/1-jenkins-seed-job/config.xml
